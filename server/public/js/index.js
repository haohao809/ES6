	...
	var Base = function () {
		...
		_createClass(Base, [{
			...
			value: function assistHandle(e) {
				...
				var $cur = (0, _jquery2.default)(e.currentTarget);
				var index = $cur.index();
				(0, _jquery2.default)('.btn-boll').removeClass('btn-boll-active');
				if (index === 0) {
					(0, _jquery2.default)('.btn-boll').addClass('btn-boll-active');
				}
				if (index === 1) {
					(0, _jquery2.default)('.btn-boll').each(function (index, item) {
						if ((0, _jquery2.default)(item).text() - 5 > 0) {
				...
				if (index === 3) {
					(0, _jquery2.default)('.btn-boll').each(function (index, item) {
						if ((0, _jquery2.default)(item).text() % 2 !== 0) {
							(0, _jquery2.default)(item).addClass('btn-boll-active');
						}
					});
				}
			...
			value: function addCode() {
				var self = this;
				var $active = (0, _jquery2.default)('.btn-boll-active').text().match(/\d{2}/g);
			...
			value: function getCount() {
				var self = this;
				var active = (0, _jquery2.default)('.btn-boll-active').length;
				var count = self.computeCount(active, self.cur_play);
			...
			value: function getRandomCode(e) {
				...
				var num = (0, _jquery2.default)(target).attr('count');
		...
		_createClass(Timer, [{
			...
			value: function countdown(end, update, handle) {
				...
				if (end - now < 0) {
					...
					var countdownTime = end - now;
					var day = 1000 * 60 * 60 * 24;
					var hour = 100 * 60 * 60;
					var min = 1000 * 60;
					var sec = 1000;
					var d = Math.floor(countdownTime / day);
					var h = Math.floor((countdownTime - d * day) / hour);
					var m = Math.floor((countdownTime - d * day - h * hour) / min);
					var s = Math.floor((countdownTime - d * day - h * hour - m * min) / sec);
					var res = [];
					if (d > 0) {
						res.push('<em>' + d + '</em>\u5929');
					}
